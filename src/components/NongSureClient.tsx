...
export default function NongSureClient({campDetail,token,user}:{campDetail:InterCampFront,token:string,user:InterUser}){
    switch (campDetail.registerModel) {
        ...
        case "noInterview": {
          return (
            <div>
              <Link href={getValue(campDetail.nongPassIds, user._id)}>
                Link
              </Link>
              <FinishButton
